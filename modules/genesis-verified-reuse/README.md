...
Reuse a current draft without inheriting its approval.

**CommonJS · Built-in modules only · Node.js 22.23.2 / 24.14.0 · MIT**

## Use it when

Avoiding repeated read-only draft work when contracts and source bytes match.
...
```js
const { buildFingerprint, admitCandidate, decideReuse } = require('./');
```
...
```mermaid
flowchart LR
  A["Bind exact inputs"] --> B["Recheck persisted bytes"] --> C["Return a draft"]
```
...
Receipt integrity is not authenticated authorship. Reuse always needs fresh task review. Dynamic work is excluded. Prior verified artifacts require the configured Astra Ultra receipt shape; unreviewed candidates still remain drafts. The caller owns persistence and a stable source tree.
