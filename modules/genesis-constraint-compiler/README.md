...
Turn explicit task effects into concrete review checks.

**CommonJS · Built-in modules only · Node.js 22.23.2 / 24.14.0 · MIT**

## Use it when

Teaching a worker a narrow invariant and its counterexample before a review.
...
```js
const { compileConstraints } = require('./');
```
...
```mermaid
flowchart LR
  A["Declare effects"] --> B["Select candidate rules"] --> C["Execute real checks"]
```
...
The catalog is advisory. Compiling a rule executes no checks, grants no authority and promotes no lesson. Unknown effects, contradictions and budget omissions remain visible gaps.
